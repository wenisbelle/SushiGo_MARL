"""Run Sushi Go matches between trained DQN and Deep CFR policies.

This script is a lightweight evaluator for the models trained in this repo.
By default it pits the DQN checkpoint against the Deep CFR average-policy
checkpoint in a 2-player game and prints the final scores.

Example:
    python3 test/play_models.py \
        --n-players 2 \
        --dqn-model models/DQN/sushi_go_qnet_2_players.pt \
...
def run_game(env, seat_policies, dqn_actor, deepcfr_model, rng, stochastic_deepcfr, verbose=False, seed=None):
    ...
    while env.agents:
        ...
        dqn_batch = None
        if dqn_actor is not None and any(seat_policies[i] == POLICY_DQN for i in range(len(active_agents))):
...
def build_seat_policies(n_players, dqn_seat, deepcfr_seat):
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run DQN vs Deep CFR matches in Sushi Go")
    ...
    parser.add_argument("--dqn-model", type=str, default="models/DQN/sushi_go_qnet_2_players.pt")
    parser.add_argument(
        "--deepcfr-model",
        type=str,
        default="models/DeepCFR/sushi_go_deep_cfr_policy_2_players.pt",
    )
    parser.add_argument("--dqn-seat", type=int, default=1, help="seat index controlled by DQN")
    parser.add_argument(
        "--deepcfr-seat",
        type=int,
        default=0,
...
def main():
    args = parse_args()
    if not (0 <= args.dqn_seat < args.n_players):
        raise ValueError("--dqn-seat must be within the player range")
    ...
    dqn_actor = load_dqn_model(args.n_players, obs_dim, args.dqn_model)
    deepcfr_model = load_deepcfr_model(obs_dim, args.deepcfr_model)

    seat_policies = build_seat_policies(args.n_players, args.dqn_seat, args.deepcfr_seat)
    ...
    for game_idx in range(1, args.games + 1):
        ...
        if len(winners) == 1:
            ...
            if winner_policy == POLICY_DQN:
                wins["dqn"] += 1
                victory_margins.append(dqn_score - deepcfr_score)
            elif winner_policy == POLICY_DEEPCFR:
                wins["deepcfr"] += 1
                victory_margins.append(deepcfr_score - dqn_score)
        else:
            wins["tie"] += 1
